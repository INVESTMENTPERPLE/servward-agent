...
def send_push(title: str, body: str, data: dict = None, category: str = "SW_ALERT") -> bool:
...
def cmd_claude_sessions(_args: dict) -> dict:
    ...
    for d in live:
        ...
        detail = ""
        # Lo que dicen los hooks manda: «te espera» también para sesiones interactivas.
    ...
    for path in glob.glob(os.path.join(CLAUDE_DIR, "jobs", "*", "state.json")):
        ...
        # herramienta, no uno que espera respuesta: solo cuenta como «te espera» si está
        # parado de verdad (tempo blocked/idle y nada en vuelo).
...
_TMUX_WATCH_WAKE: dict = {}       # (socket, target) → época de la última tecla/rueda: mirar rápido

def _tmux_watch_wake(sock: str, target: str):
    """Una tecla o rueda acaba de entrar en el pane: el vigilante mira a ritmo rápido."""
...
def _tmux_watch_loop(key: tuple):
    ...
    while True:
        ...
        if sig != last:
            ...
            for wid in ids:
                publish(wid, "ok", dict(data, watch=wid))
        # Ritmo: rápido mientras la pantalla cambia o acaba de entrar una tecla; en reposo
        # nunca más de TMUX_WATCH_IDLE_S, para que el primer cambio no llegue con retraso.
...
def cmd_tmux_scroll(args: dict) -> dict:
    """Desplaza lo que corre DENTRO del pane (apps a pantalla completa como Claude Code, que
    tmux no guarda en su historial): manda `n` pasos de rueda de ratón (`dir` up/down) si
    la app sigue el ratón; si no, Página arriba/abajo."""
    blocked = _claude_control_blocked() if ALLOW_CLAUDE_CONTROL else \
    ...
    try:
        ...
        if str(args.get("copy") or "") == "1" and info[3] != "1":
            # copy=1 (terminal en vivo con el `mouse` de tmux apagado): el historial de tmux por su
            # modo copia, que no depende de ese ajuste. Antes la app mandaba flechas, que en una
            # shell recorren los comandos. `-e`: el modo copia se cierra solo al volver abajo.
            in_mode = info[4] == "1"
            ...
            # Rueda SGR: ESC [ < 64 (arriba) | 65 (abajo) ; x ; y M, en el centro del pane.
...
# APNs (push-type liveactivity) con un semáforo: working (rojo, pensando),
# needs_you (amarillo, espera respuesta), done (verde, ha acabado). SessionEnd la cierra.
...
def cmd_claude_live_register(args: dict) -> dict:
    ...
    with _CLAUDE_LIVE_LOCK:
        ...
        for old in sorted(d, key=lambda k: d[k].get("ts", 0))[:-20]:
            d.pop(old, None)
        _live_save(d)
    log.info("LIVE_REGISTER session=%s", sid[:8])
...
def send_live_activity(token: str, state: str, detail: str, name: str,
                       end: bool = False, alert: Optional[tuple] = None) -> bool:
    ...
    try:
        ...
        log.error("LIVE_ERROR %s", e)
        return False
    if code == "200":
        log.info("LIVE_SENT  state=%s  name=%r%s", state, name, " (end)" if end else "")
        return True
    log.warning("LIVE_FAIL  http=%s  state=%s", code, state)
...
# Para las sesiones interactivas la ficha no dice «bloqueada»: lo sabemos por los eventos.
# needs_you → la lista la pone en «te espera»; working/done afinan estado y último detalle.
...
def _attention_for(ident: str):
    """Estado de atención de una sesión (por cualquiera de sus ids) o None."""
    with _ATTENTION_LOCK:
...
def _claude_pane_for(ident: str):
    ...
    for d in live:
        ...
        if fam & aliases:
            sess, _, rest = str(d["tmux"]).partition(":")       # "sess:@1.%1"
            pane_id = rest.rsplit(".", 1)[-1]
            # Resolver el socket: el que tenga ese pane (un socket sin esa sesión devuelve
            # vacío o error, no hay que fiarse de la primera respuesta).
            for sock in _tmux_sockets():
                try:
...
def cmd_claude_answer(args: dict) -> dict:
    """Responde a una sesión desde el aviso: si corre en un terminal, teclea el texto (+Intro,
    o solo Intro con yes=1 para aceptar lo resaltado); si es en segundo plano, claude_reply."""
    blocked = _claude_control_blocked()
    if blocked:
        return blocked
    ident = str(args.get("id") or "").strip()
    ...
    if pane:
        ...
        log.info("CLAUDE_ANSWER terminal target=%s chars=%d yes=%s", target, len(text), yes)
...
# La app pide `claude_watch` con un id; cada evento del hook de esa sesión se publica en
# el topic de respuestas con req_id = ese id (state, detail), y la app recarga entonces.
...
def _claude_handle_event(rec: dict):
    ...
    # 1) Live Activity de la sesión que el móvil está siguiendo (siempre, aunque el push
    #    de avisos esté silenciado: la pidió el propio móvil).
    if name == "UserPromptSubmit":
...
def handle(raw_msg: str):
    ...
    try:
        ...
        if cmd not in DEDUP_EXEMPT and req_id not in ("", "unknown"):
            ...
            if state == "running":
                log.info("DUP_RUNNING  cmd=%s  req_id=%s", cmd, req_id)
                return
            if state == "done":
                log.info("DUP_REPUBLISH  cmd=%s  req_id=%s", cmd, req_id)
